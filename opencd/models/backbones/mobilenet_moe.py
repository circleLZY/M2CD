...
class IA_MobileNetV3_MoE(MobileNetV3):
    """Interaction MobileNetV3 backbone with Mixture of Experts.
    
    Args:
        interaction_cfg (Sequence[dict]): Interaction strategies for the stages.
            Length should be the same as the number of output layers determined by out_indices.
        arch (str): Architecture of mobilenetv3. Default: 'small'.
    ...
    def __init__(self,
        ...
        if interaction_cfg is None:
            interaction_cfg = (None,) * len(self.out_indices)
                
        assert len(interaction_cfg) == len(self.out_indices), \
            f'The length of the `interaction_cfg` ({len(interaction_cfg)}) should be same as the out_indices ({len(self.out_indices)}).'
        
        # cross-correlation
        self.ccs = []
        for i, idx in enumerate(self.out_indices):
            ia_cfg = interaction_cfg[i]
            out_channels = stage_channels[idx]
            
            if ia_cfg is None:
                ia_cfg = dict(type='TwoIdentity')
            elif ia_cfg == "MoE_layer_3":
                # 定义稀疏MoE的配置
                ia_cfg = dict(
                    type='MoE_layer_3',
                    moe_cfg=dict(
                        ...
                        in_channels=out_channels,  # 根据当前阶段设置输入通道数
                        mid_channels=out_channels * 4,  # FFN中间层维度
    ...
    def _get_stage_channels(self):
        """获取MobileNetV3各层的输出通道数"""
        # 获取MobileNetV3的各层输出通道数
        if self.arch == 'large':
            return [16, 24, 24, 40, 40, 40, 80, 80, 80, 80, 112, 112, 160, 160, 160, 960]
        # MobileNetV3-small的各层输出通道数
        elif self.arch == 'small':
            return [16, 16, 24, 24, 40, 40, 40, 48, 48, 96, 96, 96, 576]
        else:
    ...
    def forward(self, x1, x2):
        ...
        for i, (f1, f2) in enumerate(zip(feats1, feats2)):
            # 使用稀疏MoE模块
            if isinstance(self.ccs[i], MoE_layer_3):
                # 转换为NHWC格式以符合MoE的输入要求
                f1 = f1.permute(0, 2, 3, 1)  # (N, C, H, W) -> (N, H, W, C)
                f2 = f2.permute(0, 2, 3, 1)  # (N, C, H, W) -> (N, H, W, C)
                
                f1, gate_loss1 = self.ccs[i](f1)
                f2, gate_loss2 = self.ccs[i](f2)
                
                f1 = f1.permute(0, 3, 1, 2)  # (N, H, W, C) -> (N, C, H, W)
                f2 = f2.permute(0, 3, 1, 2)  # (N, H, W, C) -> (N, C, H, W)
                
                total_gate_loss += gate_loss1 + gate_loss2
            else:
                f1, f2 = self.ccs[i](f1, f2)
            
            outs.append(torch.cat([f1, f2], dim=1))
...
@MODELS.register_module()
class IA_MobileNetV3_MoE_O2SP(IA_MobileNetV3_MoE):
    """Interaction MobileNetV3 backbone with Mixture of Experts and One-to-Some Perturbation."""
    ...
    def forward(self, x1, x2):
        ...
        # 应用交互并收集结果
        for i, (f1, f2, f3) in enumerate(zip(feats1, feats2, feats3)):
            # 使用稀疏MoE模块
            if isinstance(self.ccs[i], MoE_layer_3):
                # 转换为NHWC格式以符合MoE的输入要求
                f1 = f1.permute(0, 2, 3, 1)  # (N, C, H, W) -> (N, H, W, C)
                f2 = f2.permute(0, 2, 3, 1)  # (N, C, H, W) -> (N, H, W, C)
                f3 = f3.permute(0, 2, 3, 1)  # (N, C, H, W) -> (N, H, W, C)
                
                f1, gate_loss1 = self.ccs[i](f1)
                f2, gate_loss2 = self.ccs[i](f2)
                f3, gate_loss3 = self.ccs[i](f3)
                
                f1 = f1.permute(0, 3, 1, 2)  # (N, H, W, C) -> (N, C, H, W)
                f2 = f2.permute(0, 3, 1, 2)  # (N, H, W, C) -> (N, C, H, W)
                f3 = f3.permute(0, 3, 1, 2)  # (N, H, W, C) -> (N, C, H, W)
            ...
            # 计算引导损失
            total_guide_loss += 1e-4 * (nn.MSELoss()(f1, f3) + nn.MSELoss()(f2, f3))
            
            outs.append(torch.cat([f1, f2], dim=1))
        
        # 如果有guide_loss，则返回输出和总损失
...
@MODELS.register_module()
class IA_MobileNetV3Large_MoE(IA_MobileNetV3_MoE):
    """MobileNetV3-Large with Mixture of Experts for change detection."""
    def __init__(self, **kwargs):
        kwargs.setdefault('arch', 'large')
        super().__init__(**kwargs)


@MODELS.register_module()
class IA_MobileNetV3Large_MoE_O2SP(IA_MobileNetV3_MoE_O2SP):
    """MobileNetV3-Large with Mixture of Experts and One-to-Some Perturbation for change detection."""
    def __init__(self, **kwargs):
        kwargs.setdefault('arch', 'large')
        super().__init__(**kwargs)
...
@MODELS.register_module()
class IA_MobileNetV3Small_MoE(IA_MobileNetV3_MoE):
    """MobileNetV3-Small with Mixture of Experts for change detection."""
    def __init__(self, **kwargs):
        kwargs.setdefault('arch', 'small')
        super().__init__(**kwargs)


@MODELS.register_module()
class IA_MobileNetV3Small_MoE_O2SP(IA_MobileNetV3_MoE_O2SP):
    """MobileNetV3-Small with Mixture of Experts and One-to-Some Perturbation for change detection."""
    def __init__(self, **kwargs):
        kwargs.setdefault('arch', 'small')
        super().__init__(**kwargs)
...
@MODELS.register_module()
class MoE_layer_3(nn.Module):
    def __init__(self, moe_cfg):
        super(MoE_layer_3, self).__init__()
